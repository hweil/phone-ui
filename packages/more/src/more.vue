<template>
  <div class="phone-more" ref="container">
    <span class="phone-more-txt" ref="txt">中</span><span class="phone-more-ellipsis" ref="ellipsis" :style="{'display':show?'none':'inline-block'}">{{ ellipsis }}</span><button class="phone-more-button" v-if="showButton" :style="{ 'margin-left' : (show ? (marginLeft+'px'):collapseMarginLeft+'px')}" ref="more" @click="handleClick">{{ show ? collapseText: expandText }}</button>
  </div>
</template>
<script>
  export default {
